...
scanForm.addEventListener('submit', async e => {
  ...
  try {
    const res = await fetch(appUrl('api/import/keep/scan'), {
      method: 'POST',
...
document.getElementById('commit-btn').addEventListener('click', async () => {
  ...
  try {
    const res = await fetch(appUrl('api/import/keep/commit'), {
      method: 'POST',
    ...
    commitStatus.innerHTML = `Imported ${data.created} task(s). <a href="${appUrl('')}">Go to dashboard &rarr;</a>`;
    preview.hidden = true;
  } catch (err) {
