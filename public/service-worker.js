...
//
// The endpoints are the front controller's clean routes; the scope root is the dashboard.
...
self.addEventListener('notificationclick', event => {
  ...
  const target = event.notification.data?.url || scopeUrl('');
...
self.addEventListener('pushsubscriptionchange', event => {
  event.waitUntil((async () => {
    try {
      const res = await fetch(scopeUrl('api/notifications'), { credentials: 'include' });
      ...
      await fetch(scopeUrl('api/notifications'), {
        method: 'POST',
        credentials: 'include',
